...
esiJS.alliance.corps(allianceID) // returns a array of corporations within a alliance
...
esiJS.corporation.icon(corporationID) // returns links to the corporation icon
...
esiJS.dogma.dynamicItemInfo(itemID, typeID) // returns info on a dynamic item (eg. a railgun with a mutaplasmid)
...
esiJS.fw.leaderboards.chars() // returns top pilots in faction warfare
esiJS.fw.leaderboards.corps() // returns top corps in faction warfare
esiJS.fw.leaderboards.leaderboard() // returns top 4 factions in faction warfare
...
esiJS.market.orders(regionID, typeID, bOs, pageNum) // returns orders for a item in a region (typeID is optional)
